...
void Ads1115Label(char* label, uint32_t maxsize, uint32_t device) {
  ...
  snprintf(label, maxsize, PSTR("ADS1115"));
  if (ads1115_count > 1) {
    // "ADS1115-48":{"A0":3240,"A1":3235,"A2":3269,"A3":3269},"ADS1115-49":{"A0":3240,"A1":3235,"A2":3269,"A3":3269}
    snprintf(label, maxsize, PSTR("%s%c%02X"), label, IndexSeparator(), Ads1115[device].address);
#ifdef USE_I2C_BUS2
    if (TasmotaGlobal.i2c_enabled[1] &&                       // Second bus enabled
      ...
      snprintf(label, maxsize, PSTR("%s%c%d"), label, IndexSeparator(), Ads1115[device].bus +1);
    }
#endif  // USE_I2C_BUS2
  }
}
