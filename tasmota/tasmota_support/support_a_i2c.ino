/*
  HEXAOS - Refactor - remove ESP8266 support
...
bool I2cSetClock(uint32_t frequency, uint32_t bus) {
  ...
  // frequency == 0 => keep current configured frequency, or set default if unknown
  if (0 == frequency) {
    if (0 == I2C.frequency[bus]) {
      I2C.frequency[bus] = 400000;                        // Tasmota default I2C bus speed
    }
  } else {
    I2C.frequency[bus] = frequency;
  }
