...
class OV02C10 : CSI_Sensor
  ...
  def stream_on(on)
    ...
    if on
      ...
      # IDF stream path writes MIPI ctrl before 0x0100
      if !self.write_reg16(0x4800, 0x64) return false end
      ...
      if !self.write_reg16(0x4800, 0x01) return false end
  ...
  def common_regs()
    return [
      ...
      [self.REG_DELAY, 10],
      [0x4800, 0x01],      # LP-11 / clock lane disable like IDF reset table
      [self.REG_END, 0x00]
    ]
  end

def regs_1080p_2lane()
  self.width = 1920
  self.height = 1080
  self.format = 1
  self.bin_mode = 1
  self.mipi_clock = 408   # per-lane Mbps, nie 800
  ...
  return [
    ...
    # Tasmota stream zapínaj až v stream_on(), nie tu:
    [0x3503, 0x00],   # on-sensor auto exposure/gain ON
  ...
  def regs_custom(x, y, w, h, bin, fps, fmt)
    ...
    var exposure = 0x046C
    if exposure > (vts - 15) exposure = vts - 15 end
    if exposure < 8 exposure = 8 end
    ...
    return [
      ...
      # --- MIPI ---
      [0x4806, 0x00], [0x4813, 0x00], [0x4837, 0x14], [0x4857, 0x05],
      [0x4884, 0x04],
  ...
  def camera(cmd, idx, payload, raw)
    if cmd == "init"
      ...
      # Presná drop-in 1:1 referencia pre 1080p30 / 2-lane / RAW10
      if req_x == 0 && req_y == 0 && req_w == 1920 && req_h == 1080 && req_bin == 1 && req_fps == 30 && req_fmt == 1
        regs = self.regs_1080p_2lane()
      else
