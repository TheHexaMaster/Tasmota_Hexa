...
class OV02C10 : CSI_Sensor
  ...
  def dump_ae_regs(tag)
    var r3501 = self.read_reg16(0x3501)
    var r3502 = self.read_reg16(0x3502)
    var r3503 = self.read_reg16(0x3503)
    var r3508 = self.read_reg16(0x3508)
    var r3509 = self.read_reg16(0x3509)
    var r350A = self.read_reg16(0x350A)
    var r350B = self.read_reg16(0x350B)
    var r350C = self.read_reg16(0x350C)

    print(format(
      "OV02C10: %s 3501=%s 3502=%s 3503=%s 3508=%s 3509=%s 350A=%s 350B=%s 350C=%s",
      tag,
      self.hex2(r3501),
      self.hex2(r3502),
      self.hex2(r3503),
  ...
  def ae_apply_step(step)
    ...
    if step > 0
      ...
      if self.exposure_lines < max_exp
        ok = self.write_exposure(self.exposure_lines + exp_delta)
      elif self.analog_gain < 0xF8
        ok = self.write_analog_gain(self.analog_gain + again_delta)
      else
        ok = self.write_digital_gain(self.digital_gain + dgain_delta)
      end
    else
    ...
    if ok
      self.dump_ae_regs("AFTER_AE_STEP")
      return true
    end
  ...
  def regs_custom(x, y, w, h, bin, fps, fmt)
    ...
    return [
      ...
      # --- Exposure / Gain related static bits only ---
      # Seed samotného exposure/gain už robí apply_manual_ae_seed(),
      # aby neexistovali dve rozdielne zápisové cesty na tie isté registre.
      [0x3504, 0x0C],
      [0x3507, 0x00],
